...
#
tim1 = Timer(1)
tim1.init(period=10000, mode=Timer.PERIODIC, callback=callback_web) #timer init
#
#
...
#
while 1:
    v_act_sens_i = adc_e.raw_to_v(adc_e.read(7,0,1))
    ...
    if timercount:
        sens_corriente = int((0.485816226 * pow(v_max,3) -1.1205922* pow(v_max,2) + 5.651277 * v_max) / math.sqrt(2))
        sens_voltaje = int(suma_adc_i/index)
        consumo = sens_voltaje * sens_corriente
        
        #lcd.putstr(f'{sens_voltaje} V')
        #lcd.move_to(8,0)
        #lcd.putstr(f'{sens_corriente} A')
        #lcd.move_to(0,1)
        #lcd.putstr(f'{consumo} W')
